...
GitHub Actions 工作流位于 `.github/workflows/comfyui.yml`，推送 `comfyui` 分支或手动触发时构建。当前构建版本为 `v0.28.0`，通过 `BASE` 构建参数传入 PyTorch devel 基础镜像，构建 `linux/amd64` 镜像并推送到：

- `registry.cn-qingdao.aliyuncs.com/wod/comfyui:v0.28.0-pt2.11.0-cu13.0`
