...
- **私有库推送 Tag**：
  ...
  - **聚合多架构 Tag**：`registry.cn-qingdao.aliyuncs.com/wod/grafana:11.6.14` (由流水线自动使用 `docker buildx imagetools create` 进行聚合)
...
构建与推送任务均已高度自动化，由位于 [`.github/workflows/grafana.yml`](../.github/workflows/grafana.yml) 的 GitHub Actions 流水线托管。

### 触发机制

当推送/合并代码到 `grafana` 分支时自动触发构建。

### 核心构建逻辑

流水线会分步拉取 `amd64` 和 `arm64` 平台的基础镜像，打上精简的架构 Tag、配置时区并推送至阿里云：
...
- **多架构 Manifest 聚合**：使用 `docker buildx imagetools create` 将以上两个单架构镜像聚合成一个不带架构后缀的多架构 Manifest Tag（例如 `11.6.14`）。
