...
GitHub Actions 工作流位于 `.github/workflows/sub2api.yml`，推送 `sub2api` 分支或手动触发工作流时执行构建。当前构建版本为 `0.1.168`，会构建 `linux/amd64` 和 `linux/arm64` 镜像，并推送到：

- `registry.cn-qingdao.aliyuncs.com/wod/sub2api:0.1.168`
- `registry.cn-qingdao.aliyuncs.com/wod/sub2api:0.1.168-amd64`
- `registry.cn-qingdao.aliyuncs.com/wod/sub2api:0.1.168-arm64`
