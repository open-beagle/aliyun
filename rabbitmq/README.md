...
git switch rabbitmq && \
  git reset --hard main && \
  git push origin rabbitmq --force && \
  git switch main
```
...
本目录用于构建 Rabbitmq 镜像，基于上游项目进行定制开发，主要集成了 Beagle 内部环境的配置（如加速源、CA 证书、时区等）。

推送到 `rabbitmq` 分支时，会触发相应的 GitHub Actions 工作流构建镜像并推送到阿里云容器镜像服务。
